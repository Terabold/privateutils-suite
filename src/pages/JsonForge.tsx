...
import { ArrowLeft, Brain, Copy, Trash2, FileJson, Check, AlertCircle, Maximize2, Minimize2, Code, Zap } from "lucide-react";
            ...
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-6 duration-700">
              <Card className="glass-morphism border-primary/10 overflow-hidden relative bg-zinc-950 shadow-2xl rounded-2xl group">
                <div className="absolute top-4 right-4 flex gap-2 z-20">
                   <Button size="icon" variant="ghost" className="h-10 w-10 text-white/40 hover:text-white hover:bg-white/10 rounded-xl" onClick={copyToClipboard}>
                      <Copy className="h-4 w-4" />
                   </Button>
                   <Button size="icon" variant="ghost" className="h-10 w-10 text-destructive/40 hover:text-destructive hover:bg-destructive/10 rounded-xl" onClick={clearInput}>
                ...
                <textarea
                  ...
                  className="w-full h-[600px] bg-transparent p-10 font-mono text-sm text-primary/90 resize-none outline-none selection:bg-primary/30 leading-relaxed"
                  spellCheck={false}
                />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                 <Button onClick={formatJson} className="h-20 text-lg font-black rounded-2xl gap-3 shadow-2xl shadow-primary/20 italic uppercase tracking-tight">
                    <Maximize2 className="h-6 w-6" /> Prettify Code
                 </Button>
                 <Button onClick={minifyJson} variant="secondary" className="h-20 text-lg font-black rounded-2xl gap-3 italic uppercase border border-border/50">
              ...
              <Card className="glass-morphism border-primary/10 rounded-2xl overflow-hidden shadow-xl">
                 ...
                 <CardContent className="p-8 space-y-10">
                    <div className="grid grid-cols-2 gap-4">
                       <div className="bg-muted/5 p-5 rounded-xl border border-border/50">
