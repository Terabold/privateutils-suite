            ...
            <header className="flex flex-col gap-8">
              <div className="flex items-center justify-between flex-wrap gap-8">
                <div className="flex items-center gap-6">
                  <Link to="/">
                    <Button variant="outline" size="icon" className="h-12 w-12 rounded-xl border-border hover:bg-muted transition-all group/back">
                ...
                <Button 
                  ...
                  variant="outline" 
                  className="h-14 px-8 rounded-xl border-border bg-muted/50 text-foreground font-bold uppercase italic gap-3 hover:bg-foreground hover:text-background transition-all duration-300"
                >
              ...
              <div className="p-4 rounded-xl bg-card border border-border flex flex-wrap items-center gap-6 shadow-xl animate-in fade-in slide-in-from-top-4 duration-300 inline-flex w-full">
                <div className="flex flex-col gap-1 flex-1 min-w-[300px]">
                  <label className="text-[10px] font-bold uppercase tracking-[0.2em] text-primary px-1 opacity-60">Simulation Video Title</label>
            ...
            {!image ? (
              <div className="min-h-[500px] flex items-center justify-center animate-in fade-in zoom-in duration-300">
                <Card 
                  ...
                  className="w-full max-w-4xl aspect-video flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-border text-center transition-all cursor-pointer bg-card hover:border-primary/40 hover:bg-primary/5 shadow-xl group relative overflow-hidden"
                >
              ...
              <div className="flex flex-col gap-12">
                <AnimatePresence mode="wait">
                  {layoutMode === 'watch' && (
                    ...
                    >
                      <div ref={watchRef} className={`grid grid-cols-1 xl:grid-cols-12 gap-10 p-8 rounded-2xl transition-all duration-300 ${ytTheme === 'dark' ? 'bg-[#0f0f0f] border border-white/5' : 'bg-[#f9f9f9] border border-zinc-200 shadow-2xl'}`}>
                        <div className="xl:col-span-8 space-y-6">
                          <div className="relative aspect-video w-full bg-black rounded-lg overflow-hidden shadow-2xl border border-white/5 group select-none">
                            ...
                            <div className="absolute top-4 left-4 flex items-center justify-center gap-2 bg-primary px-4 py-1.5 rounded-full shadow-lg z-20 border border-white/10 whitespace-nowrap w-max min-w-fit badge-wrap-fix">
                              ...
                              <span className="text-[10px] font-black text-white uppercase tracking-tighter leading-none italic whitespace-nowrap">YOUR VIDEO</span>
                            </div>
                          ...
                          <div className="space-y-4">
                            <h2 className={`text-2xl font-bold tracking-tighter leading-tight italic font-display pr-4 overflow-visible ${ytTheme === 'dark' ? 'text-white' : 'text-zinc-900'}`}>{title}</h2>
                          ...
                          {[1,2,3,4,5,6].map((i) => (
                            <div key={i} className={`flex gap-2 group cursor-pointer border-t py-3 first:border-0 first:pt-0 ${ytTheme === 'dark' ? 'border-white/5' : 'border-zinc-200'}`}>
                              ...
                              <div className="space-y-1 py-0.5 overflow-hidden">
                                <h4 className={`text-sm font-bold leading-snug line-clamp-2 pr-4 overflow-visible transition-colors duration-300 ${ytTheme === 'dark' ? 'text-white/90 group-hover:text-primary' : 'text-zinc-900 group-hover:text-primary'}`}>{i === 1 ? title : getOtherTitle(i)}</h4>
                                <p className={`text-xs mt-1 font-medium ${ytTheme === 'dark' ? 'text-white/40' : 'text-zinc-500'}`}>{i === 1 ? 'Privacy Suite Labs' : `Casual YouTuber ${i}`}</p>
                              </div>
                            </div>
                          ))}
                        </div>
                  ...
                  {layoutMode === 'home' && (
                    <motion.div 
                      ...
                      transition={{ duration: 0.3 }}
                    >
                      <div ref={homeRef} className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 p-8 rounded-2xl transition-all duration-300 ${ytTheme === 'dark' ? 'bg-[#0f0f0f] border border-white/5' : 'bg-[#f9f9f9] border border-zinc-200 shadow-2xl'}`}>
                          ...
                          <div key={i} className={`flex flex-col gap-3 group cursor-pointer transition-all duration-300 ${i === 1 ? 'order-first' : ''}`}>
                            ...
                            <div className="flex gap-3 px-1 mt-1">
                              ...
                              <div className="space-y-1">
                                <h3 className={`text-[15px] font-bold leading-tight line-clamp-2 tracking-tighter pr-4 overflow-visible transition-colors duration-300 ${i === 1 ? 'text-primary' : (ytTheme === 'dark' ? 'text-white' : 'text-zinc-900')}`}>{i === 1 ? title : getOtherTitle(i)}</h3>
                  ...
                  {layoutMode === 'search' && (
                    <motion.div 
                      ...
                      transition={{ duration: 0.3 }}
                    >
                      <div ref={searchRef} className={`max-w-[1400px] mx-auto grid grid-cols-1 xl:grid-cols-[1fr_300px] gap-12 p-8 rounded-2xl transition-all duration-300 ${ytTheme === 'dark' ? 'bg-[#0f0f0f] border border-white/5' : 'bg-[#f9f9f9] border border-zinc-200 shadow-2xl'}`}>
                            ...
                            {[1,2,3,4,5,6].map(i => (
                              <div key={i} className={`flex flex-col sm:flex-row gap-6 group cursor-pointer transition-all duration-300 ${i === 1 ? 'order-first' : ''}`}>
                                ...
                                <div className="flex-1 py-1 space-y-2">
                                  <h3 className={`text-xl font-bold leading-tight line-clamp-2 italic tracking-tighter pr-4 overflow-visible transition-colors duration-300 ${i === 1 ? 'text-primary' : (ytTheme === 'dark' ? 'text-white' : 'text-zinc-900')}`}>{i === 1 ? title : getOtherTitle(i)}</h3>
                                  ...
                                  <div className="flex items-center gap-3 py-2">
                                    ...
                                    <span className={`text-[12px] font-bold uppercase tracking-tighter transition-colors duration-300 ${i === 1 ? 'text-primary' : (ytTheme === 'dark' ? 'text-white/60 group-hover:text-white' : 'text-zinc-600 group-hover:text-zinc-900')}`}>{i === 1 ? 'Privacy Suite Labs' : `Channel AI ${i}`}</span>
                                  </div>
