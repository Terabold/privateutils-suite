              ...
              <div className="flex items-center gap-6 animate-in fade-in slide-in-from-top-4 duration-500">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground mt-2 font-black uppercase tracking-[0.2em] opacity-40 text-[10px]">
                    Universal Architecture & Translation Studio • Multi-Format Data Smithing
                  </p>
                </div>
              </div>

              <div className="flex bg-background/40 p-1.5 rounded-2xl border border-white/10 backdrop-blur-sm shadow-2xl">
                <button 
                ...
                {activeTab === 'architecture' ? (
                  ...
                  <div className="space-y-8">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                      ...
                      <Button onClick={minifyJson} disabled={!isValid} variant="secondary" className="h-20 text-lg font-black rounded-2xl gap-3 italic uppercase border border-white/10 bg-card hover:bg-accent transition-all">
                  ...
                  <div className="space-y-8 animate-in fade-in zoom-in-95 duration-500">
                    <div className="flex gap-2">
                      {(['csv2json', 'json2csv'] as const).map(m => (
                        <button
                          ...
                          className={`px-6 py-3 rounded-2xl border transition-all font-black uppercase tracking-widest text-[10px] flex items-center gap-2 ${transMode === m ? 'bg-primary text-white border-primary shadow-lg shadow-primary/20' : 'bg-background/40 border-white/10 text-muted-foreground hover:bg-primary/5'}`}
                        >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      <Card className="glass-morphism border-primary/10 rounded-2xl shadow-xl bg-card p-8">
                        <CardContent className="p-0 space-y-4">
                          <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">{transMode === 'csv2json' ? 'Source CSV' : 'Source JSON'}</p>
                          <textarea
                            value={input}
                            onChange={(e) => setInput(e.target.value)}
                            placeholder={transMode === 'csv2json' ? "name,age\nJohn,30" : '[{"name":"John"}]'}
                            className="h-[400px] w-full bg-background/40 border border-white/10 rounded-xl p-6 text-xs font-mono text-foreground focus:outline-none focus:border-primary/40 custom-scrollbar shadow-inner"
                          />
                          <Button onClick={convert} className="w-full gap-2 h-12 font-bold rounded-2xl shadow-xl shadow-primary/20 bg-primary/10 text-primary border border-primary/20 hover:bg-primary/20 transition-all uppercase tracking-widest text-[10px]">
                      ...
                      <Card className="glass-morphism border-primary/10 rounded-2xl shadow-xl bg-card p-8">
                        <CardContent className="p-0 space-y-4">
                          <div className="flex items-center justify-between">
                            <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Output Artifact</p>
                          ...
                          <textarea
                            ...
                            className="h-[400px] w-full bg-background/40 border border-white/10 rounded-xl p-6 text-xs font-mono text-foreground/50 selection:bg-primary/30 custom-scrollbar shadow-inner"
                          />
                        </CardContent>
                      </Card>
                    </div>
                ...
                <Card className="glass-morphism border-primary/10 rounded-2xl overflow-x-clip shadow-xl bg-card">
                  ...
                  <CardContent className="p-8 space-y-10">
                    <div className="grid grid-cols-2 gap-4">
                      <div className="bg-background/40 p-5 rounded-2xl border border-white/10 shadow-inner">
                        <p className="text-[9px] font-black uppercase tracking-widest opacity-40 mb-1 leading-none text-muted-foreground">Characters</p>
                        <p className="text-2xl font-black italic tracking-tighter text-foreground">{input.length}</p>
                      </div>
                      <div className="bg-background/40 p-5 rounded-2xl border border-white/10 shadow-inner">
                        <p className="text-[9px] font-black uppercase tracking-widest opacity-40 mb-1 leading-none text-muted-foreground">Lines</p>
                    ...
                    {activeTab === 'architecture' && (
                      <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-500">
                        <div className="flex items-center justify-between p-4 bg-background/50 rounded-2xl border border-white/10 shadow-inner">
                          <div className="space-y-0.5">
                            <Label className="text-xs font-black uppercase tracking-widest flex items-center gap-2 text-foreground"><Wand2 className="h-3.5 w-3.5"/> Auto-Prettify</Label>
                        ...
                        <Button 
                          ...
                          variant="outline" 
                          className="w-full h-12 gap-2 text-xs font-black uppercase tracking-widest border-white/10 bg-card hover:bg-accent transition-all rounded-2xl"
                        >
