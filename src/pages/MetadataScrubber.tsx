        ...
        <div className="flex flex-col gap-10">
          <header className="flex items-center justify-between flex-wrap gap-8">
            <div className="flex items-center gap-6">
              <Link to="/">
                <Button variant="outline" size="icon" className="h-12 w-12 rounded-xl border border-border/50 hover:bg-primary/5 group/back transition-all shadow-sm">
            ...
            {file && (
               <Button onClick={() => { setFile(null); setPreview(null); setScrubbed(false); }} variant="ghost" size="sm" className="gap-2 h-10 px-5 text-[10px] font-black uppercase tracking-widest text-destructive hover:bg-destructive/10 border border-destructive/10 rounded-xl transition-all">
              ...
              <Card className="glass-morphism border-primary/10 overflow-hidden min-h-[500px] flex flex-col items-center justify-center relative bg-muted/5 rounded-2xl shadow-inner p-10">
                {preview ? (
                  <div className="relative w-full max-w-3xl group">
                    <img src={preview} className="w-full rounded-xl shadow-2xl transition-all group-hover:opacity-80 grayscale group-hover:grayscale-0 duration-500" alt="Preview" />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-muted/40 p-10 rounded-xl border border-border/50 studio-gradient">
                   ...
                   <p className="text-[11px] text-muted-foreground leading-relaxed italic font-medium opacity-80">
                     We use a **Bit-Level Redraw** technique. By re-rendering the image pixels onto a fresh canvas, all hidden metadata structures (EXIF, XMP, IPTC) are physically discarded.
                   </p>
                </div>
                <div className="bg-primary/5 p-10 rounded-xl border border-primary/10">
                   ...
                   <p className="text-[11px] text-muted-foreground leading-relaxed italic font-medium opacity-80">
                     Standard photos contain your exact **GPS coordinates**, camera model, and time taken. Sharing scrubbed files is essential for online privacy.
                   </p>
                </div>
              </div>
            </div>
                   ...
                   {file ? (
                     <div className="space-y-8">
                        <div className="space-y-4">
                           <div className="p-5 bg-background/50 rounded-xl border border-border/50 flex items-center gap-4">
                              <MapPin className="h-5 w-5 text-destructive" />
                              <div>
                                 <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground">GPS Location</p>
                                 <p className="text-xs font-black">DETECTION: HIGH RISK</p>
                              </div>
                           </div>
                           <div className="p-5 bg-background/50 rounded-xl border border-border/50 flex items-center gap-4">
                           ...
                           <div className="p-5 bg-background/50 rounded-xl border border-border/50 flex items-center gap-4">
                              ...
                              <div>
                                 <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground">Device ID</p>
                                 <p className="text-xs font-black">DETECTION: HIGH RISK</p>
                              </div>
                           </div>
                        </div>
