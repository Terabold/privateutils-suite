...
const ToolCard = ({ title, description, icon, to, gradient = "from-primary to-accent", themeClass }: ToolCardProps) => {
  return (
    <Link to={to} className={`group/card-link block h-full select-none cursor-pointer ${themeClass}`}>
      <Card className={`h-full border-border/50 bg-white/5 dark:bg-zinc-900 hover:bg-white/10 dark:hover:bg-zinc-800 transition-all duration-300 rounded-2xl overflow-hidden relative z-30 group-hover/card-link:border-primary/50 group-hover/card-link:shadow-2xl`}>
